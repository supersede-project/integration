...
//    }
    ...
    public void testAddFeaturesToProjectById() throws Exception{
    	...
    	feature1.setName("Atos test: New login form");
    	...
    	feature2.setName("Atos test: Welcome page");
    	...
    	feature3.setName("Atos test: Mobile version");
		...
		boolean result = proxy.addFeaturesToProjectById(payload, projectId);
		Assert.isTrue(result);
		
		for (FeatureWP3 feature: payload.getFeatures()){
			result = proxy.deleteFeatureByIdOfProjectById(feature.getId(), projectId);
			Assert.isTrue(result);
		}
    }
}
